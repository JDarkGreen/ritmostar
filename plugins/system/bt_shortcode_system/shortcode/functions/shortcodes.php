...
class Bt_Shortcodes {
			...
			for($i = 0; $i < count ( $slides ); $i ++) {
				...
				if ($isAdmin) {
					...
					if($atts['crop'] == 'yes'){
						...
						$slides [$i] ['src'] = Bt_shortcodeHelper::createThumb ($id, $slides [$i] ['src'], $atts ['thumbnail_width'], $atts ['thumbnail_height'], $atts ['crop_center'], $atts ['quality'] );
					}
				}
	...
	public static function lightbox($atts = null, $content = null) {
        ...
        $button = '[' . Bt_shortcodeHelper::getPrefix() . 'button';
        ...
        $button .= ']' . $content . '[/' . Bt_shortcodeHelper::getPrefix() . 'button]';
		...
		return '<span class="bt-lightbox ' . bt_ecssc ( $atts ) . '" data-mfp-src="' . $atts ['src'] . '" data-mfp-type="' . $atts ['type'] . '">' . do_shortcode ( $button ) . '</span>';
	}
	...
	public static function actionbox($atts = null, $content = null){
		...
		$button .= '<a href="' . $atts ['button_link'] . '" target="' . $atts ['button_target'] . '">' . $span . '</a>';
		$button .= '</div>';
