...
/**
 * @version $Id: readlesstext.php 145 2012-04-17 17:02:59Z parvus $
 */
...
class ReadLessTextExpand
{
   ...
   * @param string $plaintext RO. The plain text version of the full, unshortened article.
   * @param string $dateFormat Determines the format for the date fields to expand
   *   in the pre- and suffixes.
  ...
  function SetExpandables( $article, $plaintext, $dateFormat, $overrides = array() )
  {
    ...
    $categoryId = ReadLessTextHelper::GetCategoryId( $article );
    ...
    $this->_expandables = array(
        ...
        '{title}' => $title,
        '{id}' => ReadLessTextHelper::GetArticleId( $article ),
...
//       $before = $profiler->mark( ' Profiler ' );
      ...
      if ( ReadLessTextHelper::Filter( $article, $this->params, $options, $activeByDefaultOnAllContentItems, $extraDiscoverInfo ) )
      {
        ...
        else
       {
          ...
          {
            /* First try to pop an image from the shortened text.
             * If none appropriate could be found, try to find one in the full
             * unshortened article. There is a risk in this approach: if the
             * pre- or suffix contains an image, has already been added and
             ...
             */
            $imageHtml = $this->_PopImage( $article->text, $article->title, $this->_articleUrl, $expand );
            if ( !/*NOT*/$imageHtml )
            {
              $imageHtml = $this->_PopImage( $this->_htmltext, $article->title, $this->_articleUrl, $expand );
            }
            /* If $imageHtml is the empty string now, one of the following happened:
             * - images may not be moved
             * - no images found
             * - no acceptable image found
             * If a thumbnail was desired, and no image was found, try to find a default thumbnail.
             */
            if ( !/*NOT*/$imageHtml )
            {
              $imageHtml = $this->_GetDefaultThumbnail( $article->title, $this->_articleUrl );
            }
          }
...
//       $after = $profiler->mark( ' Profiler ' );
//       $article->text .= "<br/>Before: $before<br/>After: $after";
//       $article->introtext = $article->text;
//       $article->fulltext = '';
    }
     ...
     * @param article $article The properties introtext, text, fulltext,
     *   readmore will have been set or adapted.
     * @param params $params the parameters to use
     * @return mixed boolean false to indicate article may not be shortened,
     *   or a number indicating the nr of plaintext characters to retain.
     ...
     */
    function _PrepareArticleText( &$article, $params )
    {
...
//         echo "<br/>Lengths of article " . $article->id;
//         foreach ( array( 'char', 'word', 'paragraph', 'sentence' ) as $lengthUnit )
//         {
//           $end = true;
//           echo "<br>$lengthUnit = ". ReadLessTextHelper::DetermineLength( $this->_htmltext, $lengthUnit, $end );
//           if ( !/*NOT*/$end ) echo " OPEN ENDING. ";
//         }
//         echo "<hr>";

        $end = true;
        $length = ReadLessTextHelper::DetermineLength( $this->_htmltext, $this->_lengthUnit, $end );
     ...
     */
    function _GetParams( &$article, $length, $expand )
    {
      ...
      if ($version->RELEASE >= '1.6')
      {
        $this->_articleUrl = JRoute::_( ContentHelperRoute::getArticleRoute( $article->id, $article->catid ) );
      }
      else
      {
        /* J1.5 */
        $this->_articleUrl = JRoute::_( ContentHelperRoute::getArticleRoute( $article->id, $article->catid, $article->sectionid ) );
      }
      ...
      $expand->SetExpandables( $article, $this->_plaintext, $this->params->get( 'prefixDateFormat', '%m/%d' ), array( '{url}' => $this->_articleUrl ) );
      ...
      $expand->SetExpandables( $article, $this->_plaintext, $this->params->get( 'suffixDateFormat', '%m/%d' ), array( '{url}' => $this->_articleUrl ) );
     ...
     * @param $articleTitle the title of the article. Used as alternative text
     * for the image.
     * @param $articleUrl The full routed article's URL. Used for linking
     * the returned image.
     ...
     * string otherwise.
     */
    function _GetDefaultThumbnail( $articleTitle, $articleUrl )
    {
      $imageHtml = '';
      ...
      */
      ...
      $minimum[ 'height' ] = 0;
      $minimum[ 'ratio' ] = 0;
      $imageUrl = ReadLessTextHelper::GetThumbnail( $this->_defaultThumbnail, $minimum, $this->_crop,
          $this->_thumbWidth, $this->_thumbHeight, $this->_cacheTime );
      if ( $imageUrl )
      {
        $attributes = $this->_GetImageAttributes( $articleTitle, $this->_thumbWidth, $this->_thumbHeight );
     ...
     * suitable for display.
     * @param $articleText the string to search in and to adapt.
     * @param $articleTitle the title of the article. Used as alternative text
     * for the image.
     * @param $articleUrl The full routed article's URL. Used for linking
     * the returned image.
     ...
     * string otherwise.
     */
    function _PopImage( &$articleText, $articleTitle, $articleUrl )
    {
      $imageHtml = '';
      ...
      while ( $matchCount )
      {
        ...
        $imageUrl = ReadLessTextHelper::GetThumbnail( $imageUrl, $this->_minimum, $this->_crop, $this->_thumbWidth, $this->_thumbHeight, $this->_cacheTime );
        if ( $imageUrl )
        {
          $attributes = $this->_GetImageAttributes( $articleTitle, $this->_thumbWidth, $this->_thumbHeight );
          $imageHtml = '<a href="' . $articleUrl . '"><img src="' . $imageUrl. '"' . $attributes . '/></a>';
     ...
     */
    function _GetImageAttributes( $alt, $width = 0, $height = 0 )
    {
      $attributes = '';
      $styleValue = '';
      ...
      if ( $margin or ( $margin[0] !== '-'/* a negative number start with -*/ ) )
      {
     ...
     * admin-configurable plaintext character count, all the opened and
     * still-unclosed tags are then closed.
     * @note The prefix will not be added in this function.
     * @note The suffix may be added too. @c _suffixIsAdded will be set @c true
     *   in that case.
     ...
     */
    function _CutOff( $text, $cutOffLength )
    {
      $openTags = array();
      $result = "";
      ...
      */
      ...
      while ( $continue )
      {
        ...
        if ( !/*NOT*/$matchCount or ( count( $matches ) < 4 ) )
        {
           ...
           * Fetch the remainder of text, and set the needed variables to soem dummy value,
           * do that the loop can finish correctly.
            */
        ...
        {
          /* Add plaintext */
          $currentUnitHasEnded = true;
          $length = ReadLessTextHelper::DetermineLength( $plainText . $fullTag, $this->_lengthUnit, $currentUnitHasEnded );
          if ( $currentUnitIsOngoing and $currentUnitHasEnded )
          {
            if ( !/*NOT*/ $length )
            {
              $length = 1;
          ...
          if ( $length and !/*NOT*/$currentUnitHasEnded )
          {
            $length--;
          }
          $currentUnitIsOngoing = !/*NOT*/ $currentUnitHasEnded;
     ...
     * Once set to true, the variable may never be set to false again.
     */
